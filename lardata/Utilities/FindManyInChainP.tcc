...
namespace lar {
  ...
  namespace details {
    ...
    struct get_type<N, First, Others...> {
      static_assert(N <= sizeof...(Others), "No type in the specified position.");
      using type = get_type_t<N - 1, Others...>;
    }; // get_type<>

    template <typename First, typename... Others>
    struct get_type<0U, First, Others...> {
      using type = First;
    };
    ...
    struct TupleAppender {
      ...
      static auto append(Tuple&& t, T const& value)
      {
        return TupleAppender<(NCopies - 1)>::append(
          std::tuple_cat(std::forward<Tuple>(t), std::make_tuple(value)), value);
      }
    }; // TupleAppender<>
    ...
    struct TupleAppender<0> {
      template <typename Tuple, typename T>
      static decltype(auto) append(Tuple&& t, T const&)
      {
    ...
    art::InputTag tagFromProductID(art::ProductID const& id, Event const& event)
    {
      ...
      if (pd != nullptr) { return pd->inputTag(); }
    ...
    public:
      ...
      SimpleDataIndex(BeginIter begin, EndIter end)
      {
        init(begin, end);
      }
    ...
    private:
      ...
      struct KeyComparer {
        bool operator()(Value_t const& a, Value_t const& b) const { return a < b; }
        bool operator()(IndexElement_t const& a, Value_t const& b) const { return *(a.first) < b; }
        bool operator()(Value_t const& a, IndexElement_t const& b) const { return a < *(b.first); }
    ...
    private:
      ...
      struct KeyComparer {
        bool operator()(Key_t const& a, Key_t const& b) const { return a < b; }
        bool operator()(IndexElement_t const& a, Key_t const& b) const { return a.first < b; }
        bool operator()(Key_t const& a, IndexElement_t const& b) const { return a < b.first; }
        bool operator()(IndexElement_t const& a, IndexElement_t const& b) const
        {
          return a.first < b.first;
        }
    ...
    auto makeIndex(Coll& data, KeyExtractor&& getKey)
    {
      ...
      return DataIndex<Key_t, Value_t>(data, std::forward<KeyExtractor>(getKey));
    }
    ...
    namespace AssociationFinderBase {
      ...
      struct SourceIDinfo_t : public std::tuple<Key, std::size_t> {
        ...
        struct Comparer_t {
          bool operator()(SourceIDinfo_t const& a, SourceIDinfo_t const& b) const
          {
            return a.key() < b.key();
          }
          bool operator()(SourceIDinfo_t const& a, Key_t const& b) const { return a.key() < b; }
          bool operator()(Key_t const& a, SourceIDinfo_t const& b) const { return a < b.key(); }
      ...
      struct SourceVector_t : private std::vector<SourceIDinfo_t<Key>> {
        ...
        template <typename BeginIter, typename EndIter>
        SourceVector_t(BeginIter b, EndIter e)
        {
          assign(b, e);
        }

        /// Sorts the vector according to keys.
        void sort() { std::sort(Base_t::begin(), Base_t::end(), Info_t::comparer()); }

        /// Constructs a new element at the end of the vector.
        void emplace_back(Key_t const& key, std::size_t pos) { Base_t::emplace_back(key, pos); }
      ...
      {
        return appendToTuple<(NTags - sizeof...(InputTags))>(std::make_tuple(tags...), defTag);
      } // makeTagsTuple()
      ...
      struct Connections : private std::pair<A, std::vector<B>> {
        ...
        struct Comparer_t {
          bool operator()(Key_t const& a, Key_t const& b) const { return a < b; }
          bool operator()(Key_t const& a, Connections_t const& b) const { return a < b.key(); }
          bool operator()(Connections_t const& a, Key_t const& b) const { return a.key() < b; }
        ...
        bool operator<(Connections_t const& b) const { return comparer()(*this, b); }
      ...
      public:
        ...
        ConnectionList(KeysFromTag, BeginIter b, EndIter e)
        {
          ...
          std::transform(b, e, std::back_inserter(data()), [](auto const& key) {
            return Connections_t(key, {});
          });
        }

        /// Initializes with no connection for each of the keys from a list.
        template <typename Other>
        ConnectionList(KeysFromOtherListTag, ConnectionList<Key, Other> const& source)
        {
          data().reserve(source.size()); // hope this is fast
          using std::cbegin;
          using std::cend;
          std::transform(cbegin(source),
                         cend(source),
        ...
        std::vector<Connected_t> values() &&
        {
          ...
          std::transform(data().begin(), data().end(), std::back_inserter(result), [](auto& conn) {
            return std::move(conn.connectedTo());
          });
          return result;
        } // values()
      ...
      public:
          ...
          ConnectionList<TargetPtr_t, FurtherPtr_t>&& other) const
        {
          ...
          auto const match = makeIndex(BC, [](auto const& elem) { return elem.key(); });
          ...
          for (std::size_t iA = 0; iA < AB.size(); iA++) {
            ...
            for (auto const& key : BsForA) {
              ...
              if (CsForA.empty())
                CsForA = std::move(CsForB); // move
              else
                CsForA.insert(CsForA.end(), cbegin(CsForB), cend(CsForB));
      ...
      {
        return PtrConnectionManager<typename A::value_type, typename B::value_type>(
          std::move(data));
      }
    ...
    struct AssociationFinder {
      ...
      using Result_t = AssociationFinderBase::ConnectionList<art::Ptr<Source>, TargetPtr_t>;
      ...
      template <typename Assns>
      static ResultFromAssns_t<Assns> findFromDataProduct(art::ProductID const& sourceID,
                                                          std::size_t nSources,
                                                          Assns const& assns)
      {
        ...
        ResultFromAssns_t<Assns> result(KeysFromIndex, nSources, [id = sourceID](std::size_t i) {
          return SourcePtr_t(id, i, nullptr);
        });
      ...
      template <typename Source, typename IndexBegin, typename IndexEnd, typename Event>
      static Result_t<Source> findSingle(art::ProductID const& sourceID,
                                         IndexBegin sbegin,
                                         IndexEnd send,
      ...
      static auto findWithRange(PtrCollBegin sbegin,
                                PtrCollEnd send,
                                Event const& event,
                                art::InputTag const& tag)
      {
        ...
        auto const& assns = *(event.template getValidHandle<Assns_t<Source_t>>(tag));
      ...
      /// source art pointer.
      template <typename PtrCollBegin, typename PtrCollEnd, typename Event>
      static auto findWithRange(PtrCollBegin sbegin, PtrCollEnd send, Event const& event)
      {
        /*
         * The strategy of this implementation is:
         *
        ...
        std::map<art::ProductID, SourceVector_t<std::size_t>> sourcesLeft;
        ...
        for (auto const& sourcesWithinID : sourcesLeft) {
          ...
          art::InputTag tag = tagFromProductID<std::vector<Source_t>>(sourceID, event);
    ...
    auto findAssociations(Handle&& handle, Event const& event, art::InputTag const& assnsTag)
    {
      using Finder = AssociationFinder<Target>;
      using Assns_t = typename Finder::template AssnsFromHandle_t<std::decay_t<Handle>>;
    ...
    auto findAssociations(Handle handle, Event const& event, lar::SameAsDataTag)
    {
      ...
      return findAssociations<Target>(std::forward<Handle>(handle), event, tag);
    ...
    auto findAssociations(PtrColl const& coll, Event const& event, lar::SameAsDataTag)
    {
      using std::cbegin;
      using std::cend;
      return AssociationFinder<Target>::findWithRange(cbegin(coll), cend(coll), event);
    }
    ...
    template <typename Target,
              ...
              typename Event,
              typename = std::enable_if_t<!is_handle_v<PtrColl>>>
    auto findAssociations(PtrColl const& coll, Event const& event, art::InputTag const& tag)
    {
      using std::cbegin;
      using std::cend;
      return AssociationFinder<Target>::findWithRange(cbegin(coll), cend(coll), event, tag);
    } // findAssociations(Handle, tag)
    ...
    struct FindManyInChainPimpl {
      ...
      static_assert(Tier < Tiers, "Invalid tier: there is no type in position #N");
      ...
      static auto find(Source&& source, Event const& event, InputTags const& tags)
      {
        ...
        auto iq = makeConnectionManager(
          findAssociations<Intermediate_t>(source, event, std::get<nTag>(tags)));
        ...
        auto oq = FindManyInChainPimpl<Target, (Tier - 1U), Intermediate...>::find(
    ...
    struct FindManyInChainPimpl<Target, 0U, Intermediate...> {
      ...
      static auto find(Source&& source, Event const& event, InputTags const& tags)
      {
        ...
        // (using the last tag)
        return findAssociations<Target>(source, event, std::get<Tiers - 1>(tags));
      } // find()
...
template <typename Target, typename... Intermediate>
template <typename Source, typename Event, typename... InputTags>
auto lar::FindManyInChainP<Target, Intermediate...>::find(Source&& source,
                                                          Event const& event,
                                                          InputTags... tags)
  -> std::vector<TargetPtrCollection_t>
{
  ...
  auto const allTags = details::AssociationFinderBase::makeTagsTuple<Tiers>(
    SameAsData, std::forward<InputTags>(tags)...);

  return details::FindManyInChainPimpl<Target, (Tiers - 1), Intermediate...>::find(
           source, event, allTags)
    .values();
...
//------------------------------------------------------------------------------
template <typename Target, typename... Intermediate>
std::size_t lar::FindManyInChainP<Target, Intermediate...>::size() const noexcept
{
  return results.size();
}
