...
void util::SignalShaper::AddResponseFunction(const std::vector<double>& resp, bool ResetResponse)
{
  ...
  if (fResponseLocked) throw cet::exception("SignalShaper") << "Configuration locked.\n";
...
void util::SignalShaper::AddFilterFunction(const std::vector<std::complex<double>>& filt)
{
  ...
  if (fFilterLocked) throw cet::exception("SignalShaper") << "Configuration locked.\n";
...
void util::SignalShaper::LockResponse() const
{
  ...
  if (!fResponseLocked) {
    ...
    if (fResponse.size() == 0)
      throw cet::exception("SignalShaper") << "Response has not been configured.\n";
...
void util::SignalShaper::CalculateDeconvKernel() const
{
  ...
  if (fFilter.size() == 0)
    throw cet::exception("SignalShaper") << "Filter function has not been configured.\n";
  ...
  if (2 * (fFilter.size() - 1) != n)
    if (fFilter.size() != fConvKernel.size()) {
      throw cet::exception("SignalShaper") << __func__ << ": inconsistent size, " << fFilter.size()
                                           << " vs. " << fConvKernel.size() << "\n";
    }
