...
namespace {
  ...
  {
    if (dir == "")
      throw art::Exception(art::errors::Configuration) << "Vacuous directory name" << std::endl;
    ...
    while ((dirp = readdir(dp)) != NULL) {
      ...
      if (filename.find("bin") != std::string::npos) { files.push_back(filename); }
    }
    closedir(dp);
                        ...
                        std::vector<raw::ExternalTrigger>& extTrig)
  {
    ...
    for (int i = 0; i != nwires; ++i) {
      ...
      for (int ijk = 0; ijk < c1.samples; ++ijk) {
        if (std::abs(adclist[ijk]) < 1e-5) adclist[ijk] += 400;
      }
      ...
      if (i < 96) {
        ...
        digitList[239 - i].SetPedestal(
          400.); //carl b assures me this will never change. bjr 4/15/2009
      }
    }
...
namespace lris {
  ...
  void LArRawInputDriverLongBo::readFile(std::string const& name, art::FileBlock*& fb)
  {
    ...
    fb = new art::FileBlock(art::FileFormatVersion(1, "LArRawInput 2011a"), currentDir_);
  }
                                         ...
                                         art::EventPrincipal*& outE)
  {
    ...
    std::unique_ptr<std::vector<raw::ExternalTrigger>> etcollb(
    ...
    if (firstEventInRun) {
      ...
      outSR = principalMaker_.makeSubRunPrincipal(rn, currentSubRunID_.subRun(), tstamp);
      ...
      throw cet::exception("InconsistentEventStream")
        << "Encountered run #" << rn << " while processing events from run #"
        << currentSubRunID_.run() << "\n";
    }

    outE = principalMaker_.makeEventPrincipal(
      currentSubRunID_.run(), currentSubRunID_.subRun(), daqHeader.GetEvent(), tstamp);

    // Put products in the event.
    art::put_product_in_principal(std::move(etcollb), *outE,
                                  "daq"); // Module label
    art::put_product_in_principal(std::move(rdcollb), *outE,
                                  "daq"); // Module label
    art::put_product_in_principal(std::move(daqcollb), *outE,
                                  "daq"); // Module label
