                    ...
                    int NDF)
{
  ...
  if (n == 0) {
    ...
    BOOST_CHECK_CLOSE(double(fitter.InterceptError()), double(intercept_error), 0.1);
    BOOST_CHECK_CLOSE(double(fitter.SlopeError()), double(slope_error), 0.1);
    BOOST_CHECK_CLOSE(
      double(fitter.InterceptSlopeCovariance()), double(intercept_slope_covariance), 0.1);
    if (double(chisq) == 0.)
                       ...
                       int NDF)
{
  ...
  if (n == 0) {
    ...
    BOOST_CHECK_CLOSE(double(fitter.FitParameterError(0)), std::sqrt(double(error2[0])), 0.1);
    BOOST_CHECK_CLOSE(double(fitter.FitParameterError(1)), std::sqrt(double(error2[1])), 0.1);
    BOOST_CHECK_CLOSE(double(fitter.FitParameterError(2)), std::sqrt(double(error2[2])), 0.1);
    if (double(chisq) == 0.)
      BOOST_CHECK_SMALL(double(fitter.ChiSquare()), 1e-5);
    else
      BOOST_CHECK_CLOSE(double(fitter.ChiSquare()), double(chisq), 0.1);
