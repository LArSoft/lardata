...
struct SurfXYZTestFixture {
  SurfXYZTestFixture()
    : surf1(), surf2(0., 0., 0., 0., 0.), surf3(1., 1., 1., 0., 0.), surf4(1., 2., 2., 1., 0.1)
  {}
...
{
  BOOST_CHECK_EXCEPTION(surf1.distanceTo(surf4), cet::exception, [](cet::exception const& e) {
...
BOOST_AUTO_TEST_CASE(TrackParameters)
{
  ...
  BOOST_TEST(mom[1] == (4. * std::sin(0.1) * std::sin(1.) + 6. * std::cos(1.) -
                        2. * std::cos(0.1) * std::sin(1.)) /
                         std::sqrt(14.),
             tol);
  BOOST_TEST(mom[2] == (-4. * std::sin(0.1) * std::cos(1.) + 6. * std::sin(1.) +
                        2. * std::cos(0.1) * std::cos(1.)) /
                         std::sqrt(14.),
             tol);

  // Should throw exception (no direction).

  BOOST_CHECK_EXCEPTION(surf1.getMomentum(v, mom), cet::exception, [](cet::exception const& e) {
    return e.category() == "SurfXYZPlane";
  });
}
